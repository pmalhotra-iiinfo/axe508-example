...
public class MealRepository {
	 ...
	 * @param fromDate
	 *            - search from this date, including
	 * @param toDate
	 *            - search until this date, including
	 * @param fromTime
	 *            - search from this time, including
	 * @param toTime
	 *            - search to this time, including
	 * @return - a list of matching meals, or an empty collection if no match found
	 */
	public Long countMealsByDateTime(String username, Date fromDate, Date toDate, Time fromTime, Time toTime,String description) {
		...
		cq.where(getCommonWhereCondition(cb, username, countRoot, fromDate, toDate, fromTime, toTime,description));
	 ...
	 * @return - a list of matching meals, or an empty collection if no match found
	 */
	public List<Meal> findMealsByDateTime(String username, Date fromDate, Date toDate, Time fromTime, Time toTime,
		...
		searchQuery.where(
				getCommonWhereCondition(cb, username, searchRoot, fromDate, toDate, fromTime, toTime, description));
		...
		TypedQuery<Meal> filterQuery = em.createQuery(searchQuery).setFirstResult((pageNumber - 1) * 10)
				.setMaxResults(10);
	...
	private Predicate[] getCommonWhereCondition(CriteriaBuilder cb, String username, Root<Meal> searchRoot, Date fromDate, Date toDate,
                                                Time fromTime, Time toTime,String description) {
